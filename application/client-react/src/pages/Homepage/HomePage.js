...
import {
  FaWallet, FaFileSignature, FaStamp, FaShieldAlt,
  FaUsers, FaLock, FaNetworkWired, FaEyeSlash,
  FaQuestionCircle, FaClock, FaUserShield
} from 'react-icons/fa';
import styled from 'styled-components';

import {
  HeroSection, HeroText, HeroTitle, HeroDesc, ButtonGroup,
  PrimaryButton, SecondaryButton, HeroCard, CardHeader,
  CardTitle, CardBox, CardFooter
} from './Style/HeroSection';

import {
  StepContainer, StepTitle, StepDesc, StepTimeline,
  StepItem, IconCircle, StepLabel, StepText, Connector
} from './Style/StepTimeline';

import {
  WhySection, FeatureGrid, FeatureCard
} from './Style/WhySection';

import {
  WillPreviewSection, PreviewHeader, PreviewTitle, PreviewSub,
  PreviewContent, PreviewLeft, PreviewRight, PreviewCard,
  PreviewCardHeader, StatusTag, PreviewDate, PreviewCardTitle,
  PreviewCardBody, PreviewCardFooter, PreviewTag, TagLabel,
  TagValue, PreviewInfoBlock
} from './Style/WillPreviewSection';

import {
  TestimonialSection, TestimonialHeader, TestimonialTitle,
  TestimonialSub, TestimonialGrid, TestimonialCard,
  UserInfo, UserAvatar, UserMeta, UserName, UserJob, UserQuote
} from './Style/TestimonialsSection';

import {
  GetStartSection, GetStartTitle, GetStartSub, GetStartGrid,
  GuideBlock, FaqBlock, GuideTitle, GuideItem, GuideText,
  GuideTextTitle, GuideTextSub, CtaBox, CtaButton,
  FaqTitle, FaqQuestion, FaqQuestionTitle, FaqQuestionText,
  SectionDivider, InquiryBox, InquiryText, InquiryTitle,
  InquirySub, InquiryButton, LegalFooterNote
...
const HomePage = () => {
  ...
  const testimonials = [
    { name: "김민수", job: "40대 회사원", image: "/images/kim.PNG", quote: "아버지가 남기신 말 한 마디, 우리 가족 모두가 함께 들을 수 있었어요. 마침표 덕분에 아버지의 마지막 뜻을 명확히 알 수 있었습니다." },
    { name: "이지현", job: "50대 자영업자", image: "/images/ei.PNG", quote: "복잡한 상속 문제로 가족 간 갈등이 있었는데, 마침표를 통해 작성한 유언장 덕분에 모든 것이 명확해졌어요. 이제 마음의 평화를 찾았습니다." },
    { name: "박준호", job: "30대 IT 전문가", image: "/images/back.PNG", quote: "기술에 익숙하지 않은 부모님을 위해 마침표를 소개해드렸어요. 너무 쉬운 인터페이스에 부모님도 직접 사용하실 수 있었고, 이제 가족 모두 안심하고 있습니다." }
  ];

  const features = [
    { title: "블록체인으로 위변조 방지", desc: "기록된 유언장은 변경이 불가해 위변조 걱정이 없습니다.", icon: FaShieldAlt },
    { title: "법적 효력 연동", desc: "공증 및 인증 기관과 연동하여 법적 효력을 인정받습니다.", icon: FaStamp },
    { title: "가족 간 분쟁 예방", desc: "명확하고 안전하게 전달되어 분쟁을 예방합니다.", icon: FaUsers },
    { title: "암호화된 보안", desc: "블록체인 기반 저장과 암호화 열람으로 보안이 강화됩니다.", icon: FaLock },
    { title: "분산 저장 시스템", desc: "유언장을 안전하게 여러 네트워크에 분산 저장합니다.", icon: FaNetworkWired },
    { title: "유언장 접근 제한", desc: "열람 권한이 있는 사용자만 접근 가능하게 설정됩니다.", icon: FaEyeSlash }
  ];
  return (
    <Container>
      {/* HeroSection */}
      <HeroSection>
        <HeroText>
          <HeroTitle>
            <span className="line1">당신의 마지막 뜻,</span>
            <span className="line2">안전하게 남기는 가장 현대적인 방법.</span>
          </HeroTitle>
          <HeroDesc>
      ...
      {/* Slider */}
      <SliderContainer>
        <ArrowButton left onClick={goToPrevSlide}><HiOutlineChevronLeft size={24} /></ArrowButton>
        ...
        <SliderWrapper style={{ transform: `translateX(-${currentIndex * 100}vw)` }}>
          ...
          <SectionWrapper>
            <StepContainer>
              ...
              <StepDesc>마침표는 최신 블록체인 기술을 활용하여 당신의 소중한 유언을 안전하게 보호하고 전달합니다.</StepDesc>
              <StepTimeline>
                <StepItem>
          ...
          <SectionWrapper>
            <WhySection>
              ...
              <StepDesc>신뢰할 수 있는 기술과 절차를 통해 삶의 마지막을 위한 최적의 유언 솔루션을 제공합니다.</StepDesc>
              <FeatureGrid>
                {features.map((item, i) => (
                  <FeatureCard key={i}>
          ...
          <SectionWrapper>
            <WillPreviewSection>
              <PreviewHeader>
                ...
                <PreviewSub>마침표의 디지털 유언장은 직관적인 인터페이스로 중요한 정보를<br />한눈에 확인할 수 있습니다.</PreviewSub>
              </PreviewHeader>
              <PreviewContent>
                <PreviewLeft>
                  <PreviewCard>
        ...
        <ArrowButton onClick={goToNextSlide}><HiOutlineChevronRight size={24} /></ArrowButton>
      </SliderContainer>
